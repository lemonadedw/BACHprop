...
async function main() {
  ...
  try {
    ...
    // Read existing download information from file
    let download_information = [];
    try {
        const existingData = fs.readFileSync(download_information_file, 'utf8');
        download_information = JSON.parse(existingData);
    } catch (error) {
        // If file doesn't exist or is invalid, start with empty array
        download_information = [];
    }
    
    // Set download_count based on existing data
    download_count = download_information.length > 0 
        ? Math.max(...download_information.map(item => item.id || 0))
        : 0;
    ...
    for (const target of genre_urls) {
        ...
        for (let i = 1; i <= max_pages; i++) {
            // Musescore pagination format: ?page={page_number} or &page={page_number} if URL already has query params
            const separator = url.includes('?') ? '&' : '?';
            await page.goto(`${url}${separator}page=${i}`, { waitUntil: 'networkidle2' });
    ...
    for (const {url, genre, page: pageNum} of score_links) {
        // Skip if URL already exists in download_information
        if (download_information.some(item => item.url === url)) {
        ...
        try {
            ...
            if (!midiButtonFound) {
                ...
                // Wait for download to complete and get filename
                await new Promise(resolve => setTimeout(resolve, 2000)); // Give time for download to complete
                
                // Find the newly downloaded file
                const filesAfter = fs.readdirSync(downloadsDir);
                const newFiles = filesAfter.filter(f => !filesBefore.has(f));
                const downloadedFilename = newFiles.length > 0 ? newFiles[0] : null;
                ...
                // Write to file immediately to preserve progress if script errors out
